...
export default function Navbar({ initialContent = {}, initialViewport = 'desktop' }: NavbarProps) {
  ...
  const [stickySpacerHeight, setStickySpacerHeight] = useState(0);
  ...
  const headerStyle = model.headerTransparent
    ? {
        background:
          'linear-gradient(180deg, rgba(0, 0, 0, 0.78) 0%, rgba(0, 0, 0, 0.62) 100%)',
    ...
    : {
        ...resolveBackgroundStyle(model.headerBgColor, '#000000'),
  ...
  useEffect(() => {
    if (!model.stickyHeader) {
      setStickySpacerHeight(0);
      return;
    }

    const updateHeight = () => {
      const measured = headerRef.current?.offsetHeight ?? 0;
      setStickySpacerHeight(measured);
    };
  ...
  return (
    <>
      {model.stickyHeader ? <div aria-hidden style={{ height: stickySpacerHeight }} /> : null}

      <header
        ref={headerRef}
        className={cn(
          model.stickyHeader ? 'fixed inset-x-0 top-0' : 'relative',
          'z-50 border-b',
      ...
      >
          ...
          <Link href="/" className="group flex items-center" aria-label={model.brandName ? `${model.brandName} Home` : 'Home'}>
            {model.logoImage ? (
              ...
              <img
                ...
                style={{
                  height: `${model.logoSize}px`,
                  width: 'auto',
                  maxWidth: `${Math.round(model.logoSize * 3)}px`,
                }}
                loading="eager"
                decoding="async"
              />
            ) : model.showBrandText && model.brandName ? (
              <span
            ...
            {model.logoImage && model.showBrandText && model.brandName ? (
              <span
        ...
        <MobileMenu
          ...
          mobileMenuCardBgColor={model.mobileMenuCardBgColor}
          mobileMenuCardBorderColor={model.mobileMenuCardBorderColor}
          mobileMenuEnableActiveHighlight={model.mobileMenuEnableActiveHighlight}
          mobileMenuActiveBgColor={model.mobileMenuActiveBgColor}
          mobileMenuActiveTextColor={model.mobileMenuActiveTextColor}
          mobileMenuIconBgColor={model.mobileMenuIconBgColor}
          mobileMenuIconColor={model.mobileMenuIconColor}
          mobileMenuIconActiveBgColor={model.mobileMenuIconActiveBgColor}
          mobileMenuIconActiveColor={model.mobileMenuIconActiveColor}
          mobileMenuRadius={model.mobileMenuRadius}
          mobileMenuText={model.mobileMenuText}
          mobileMenuBrandName={model.mobileMenuBrandName}
          mobileMenuLogoImage={model.mobileMenuLogoImage}
          mobileMenuSubtitle={model.mobileMenuSubtitle}
          mobileMenuTitle={model.mobileMenuTitle}
          mobileMenuPrimaryCtaText={model.mobileMenuPrimaryCtaText}
          mobileMenuPrimaryCtaUrl={model.mobileMenuPrimaryCtaUrl}
          mobileMenuWhatsappUrl={model.mobileMenuWhatsappUrl}
          mobileMenuInstagramUrl={model.mobileMenuInstagramUrl}
          mobileMenuYoutubeUrl={model.mobileMenuYoutubeUrl}
