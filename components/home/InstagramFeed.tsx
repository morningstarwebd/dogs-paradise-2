...
function buildGalleryItems(blocks: RawBlock[]): GalleryImageItem[] {
  return blocks
    .filter((block) => block?.type === 'gallery_image' && block.settings)
    .map((block, index) => {
...
export default function InstagramFeed({
  heading = 'Adorable Moments',
  subheading = 'A glimpse into the daily lives of our happy puppy families. Follow our journey on Instagram for a daily dose of cuteness.',
  handle = '@dogsparadice',
  blocks = [],
  section_bg_color,
  section_text_color,
  ...
  const normalizedHandle = handle.replace(/^@/, '').trim() || 'dogsparadice';

  const blockItems = buildGalleryItems(blocks);
  const defaultItems: GalleryImageItem[] = galleryImages.map((image, index) => ({
    id: `default_gallery_${index}`,
    src: image,
    alt: `Puppy gallery photo ${index + 1}`,
    row: index < 8 ? 'top' : 'bottom',
  }));
  const activeItems = blockItems.length > 0 ? blockItems : defaultItems;

  let topRowItems = activeItems.filter((item) => item.row === 'top');
  let bottomRowItems = activeItems.filter((item) => item.row === 'bottom');

  if (topRowItems.length === 0 || bottomRowItems.length === 0) {
    const midpoint = Math.ceil(activeItems.length / 2);
    topRowItems = activeItems.slice(0, midpoint);
    bottomRowItems = activeItems.slice(midpoint);
  }
  ...
  return (
    <section className="section-shell overflow-hidden bg-[#302b63]" id="photo-gallery" ref={containerRef} style={sectionStyle}>
      ...
      <div className="space-y-6">
        <motion.div style={{ x: x1 }} className="flex gap-6 pl-6">
          {topRowItems.map((item, i) => (
            <GalleryItem key={`top-${item.id}-${i}`} item={item} index={i} />
          ))}
          {topRowItems.slice(0, Math.min(4, topRowItems.length)).map((item, i) => (
            <GalleryItem key={`top-dup-${item.id}-${i}`} item={item} index={i} />
          ))}
        </motion.div>

        <motion.div style={{ x: x2 }} className="flex gap-6 pl-6">
          {bottomRowItems.map((item, i) => (
            <GalleryItem key={`bottom-${item.id}-${i}`} item={item} index={i} />
          ))}
          {bottomRowItems.slice(0, Math.min(4, bottomRowItems.length)).map((item, i) => (
            <GalleryItem key={`bottom-dup-${item.id}-${i}`} item={item} index={i} />
          ))}
        </motion.div>
      </div>
...
function GalleryItem({ item, index }: { item: GalleryImageItem; index: number }) {
  const content = (
    <>
      <Image
        src={item.src}
        alt={item.alt || `Puppy gallery photo ${index + 1}`}
        fill
  ...
  if (item.url) {
    return (
      <a
        ...
        target="_blank"
        rel="noopener noreferrer"
        className="relative h-[190px] w-[190px] shrink-0 overflow-hidden rounded-2xl group sm:h-[240px] sm:w-[240px] lg:h-[280px] lg:w-[280px]"
      >
        {content}
      </a>
    );
